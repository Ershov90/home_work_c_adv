...
// функция root для возвращения числа итераций
  ...
  *iterations = 0; // Счетчик итераций
  ...
  while (fabs(f(x) - g(x)) > eps1) {
    if (fabs(df(x) - dg(x)) < 1e-10) {
      printf("Error: Divide by zero. Exit.\n");
      return 0; 
    } else {
...
{
  double eps1 = 1e-6; //Определение точности
  double eps2 = 1e-6; 
  ...
  if (showRoots) {
    printf("Intersection points of curves:\n");
  ...
  if (showTest) {
    ...
    printf("Root of the equation f1(x) = f2(x) on line [0, 3]: %lf\n", root(f1, f2, 0, 3, eps1, df1, df2, &iterationsTest1)); 
    printf("Root of the equation f1(x) = f3(x) on line [1, 4]: %lf\n", root(f1, f3, 1, 4, eps1, df1, df3, &iterationsTest2));
    printf("Root of the equation f2(x) = f3(x) on line [1, 4]: %lf\n", root(f2, f3, 1, 4, eps1, df2, df3, &iterationsTest3));
    printf("Test function integral:\n");
    printf("Integral from f1(x) on line [0, 3]: %lf\n", integral(f1, 0, 3, eps2));
    printf("Integral from f2(x) on line [0, 3]: %lf\n", integral(f2, 0, 3, eps2));
    printf("Integral from f3(x) on line [1, 4]: %lf\n", integral(f3, 1, 4, eps2));
    printf("Number of iterations for root finding:\n");
    printf("f1(x) = f2(x): Number of iterations: %d\n", iterationsTest1);
    printf("f1(x) = f3(x): Number of iterations: %d\n", iterationsTest2);
    printf("f2(x) = f3(x): Number of iterations: %d\n", iterationsTest3);
    return 0;
  }
